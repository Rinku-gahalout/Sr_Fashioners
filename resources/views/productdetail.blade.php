    ...
    <div class="pd-layout">
      ...
      <div class="pd-gallery">
        ...
        <div class="pd-thumbs" id="thumbList">
          @forelse ($galleryImages as $i => $img)
            <div class="pd-thumb {{ $i === 0 ? 'pd-thumb--active' : '' }}"
                 data-src="{{ asset($img) }}">
        ...
        <div class="pd-main-wrap">
          ...
          @else
            <div class="pd-main-placeholder">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
              <span>No Image Available</span>
            </div>
          @endif

          <button class="pd-wishlist" id="wishlistBtn" aria-label="Add to wishlist">
            <svg id="wishHeart" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
      ...
      <div class="pd-info">
        ...
        <div class="pd-actions">
          <button class="pd-btn pd-btn--cart" type="button"
                  {{ !$inStock ? 'disabled' : '' }}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg>
            {{ $inStock ? 'Add to Cart' : 'Out of Stock' }}
          </button>
          <button class="pd-btn pd-btn--buy" type="button"
                  {{ !$inStock ? 'disabled' : '' }}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
            Buy Now
          </button>
        </div>
        ...
        <div class="pd-trust">
          <div class="pd-trust__item">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><rect x="1" y="3" width="22" height="18" rx="2"/><path d="M1 9h22"/></svg>
            <span>Secure Payment</span>
          </div>
          <div class="pd-trust__item">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
            <span>Free Delivery ₹999+</span>
          </div>
          <div class="pd-trust__item">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 .49-3.51"/></svg>
    ...
    <div class="pt-panel pt-panel--active" id="tab-info">
      <div class="pt-info-grid">
        <div>
          ...
          @else
            <p>Fashion has been creating well-designed collections since 2010. The brand offers feminine designs delivering stylish separates and statement dresses which have evolved into a full ready-to-wear collection where every item is a vital part of a woman's wardrobe.</p>
            <p>Cool, easy, chic looks with youthful elegance and unmistakable signature style. All pieces are made with the greatest attention to craftsmanship.</p>
          @endif
        </div>
      ...
      <div class="pt-table-wrap">
        <table class="pt-table">
          ...
          @if (!empty($product->wash_care))
            <tr><td>Wash Care</td><td>{{ $product->wash_care }}</td></tr>
          @endif
          @if ($subCategory)
            <tr><td>Sub Category</td><td>{{ $subCategory->name }}</td></tr>
          @endif
          @if ($categoryModel)
            <tr><td>Category</td><td>{{ $categoryModel->name }}</td></tr>
          @endif
          @if (!empty($product->sku))
            <tr><td>SKU</td><td>{{ $product->sku }}</td></tr>
          @endif
          @if (!empty($product->origin))
            <tr><td>Country of Origin</td><td>{{ $product->origin }}</td></tr>
          @endif
          {{-- Fallback rows if none of the above exist --}}
          @if (empty($product->fabric) && empty($product->fit) && empty($product->wash_care))
            <tr><td>Category</td><td>{{ $categoryModel?->name ?? 'N/A' }}</td></tr>
            <tr><td>Sub Category</td><td>{{ $subCategory?->name ?? 'N/A' }}</td></tr>
          @endif
        </table>
      </div>
    </div>
        ...
        <div class="rv-score-col">
          ...
          <div class="rv-bars">
            @php
              ...
              $starColors = [5=>'#4caf50', 4=>'#3b82f6', 3=>'#f4b400', 2=>'#f97316', 1=>'#888'];
            @endphp
            @foreach ($starData as $star => $pct)
              <div class="rv-bar-row">
                <span>{{ $star }}★</span>
                <div class="rv-bar">
                  <div class="rv-fill" style="width:{{ $pct }}%;background:{{ $starColors[$star] }}"></div>
                </div>
        ...
        <div class="rv-cards-col">
          ...
          @else
            ...
            <div class="rv-card">
              ...
              <p>Absolutely love this! The fabric is soft and the fit is perfect. Got so many compliments.</p>
            </div>
            <div class="rv-card">
              <div class="rv-card__header">
...
<section class="grid-section">
  <div class="pd-container">
    <div class="gs-header">
      ...
      <a href="{{ route('list', ['category' => $categoryModel?->slug ?? 'all']) }}" class="gs-see-all">View All →</a>
    </div>
    <div class="gs-grid">
      @foreach ($relatedProducts as $rp)
        ...
        @endphp
        <a class="gs-card" href="{{ route('product.detail', ['category' => $rpCategory, 'product_name' => $rpSlug]) }}">
          <div class="gs-card__img-wrap">
            ...
            <button class="gs-card__wish" type="button" aria-label="Wishlist" onclick="event.preventDefault();">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
            </button>
          ...
          <div class="gs-card__body">
            ...
            @php
              ...
              $rpColors = is_array($rp->colors) ? $rp->colors : (json_decode($rp->colors ?? '[]', true) ?? []);
            @endphp
  ...
  <div class="pd-container">
    ...
    <div class="gs-grid">
      @foreach ($recentlyViewed as $rv)
        ...
        @endphp
        <a class="gs-card" href="{{ route('product.detail', ['category' => $rvCategory, 'product_name' => $rvSlug]) }}">
          <div class="gs-card__img-wrap">
          ...
          <div class="gs-card__body">
            ...
            @php
              $rvSizes  = is_array($rv->sizes)  ? $rv->sizes  : (json_decode($rv->sizes  ?? '[]', true) ?? []);
              $rvColors = is_array($rv->colors) ? $rv->colors : (json_decode($rv->colors ?? '[]', true) ?? []);
            @endphp
            <div class="gs-card__meta">
              <span class="gs-card__sizes">
...
.pd-breadcrumb__current { color: var(--gold); }

/* ============================================================
   PRODUCT HERO
============================================================ */
.pd-hero { background: var(--white); padding-bottom: 60px; }
...
.pd-thumb--active::after { border-left-color: var(--gold); }
...
.pd-main-img.pd-img-fade { opacity: 0; transform: scale(1.02); }
...
.pd-wishlist svg { width: 20px; height: 20px; }
.pd-wishlist:hover, .pd-wishlist.pd-wishlist--active { color: #e53e3e; }
.pd-wishlist.pd-wishlist--active svg { fill: #e53e3e; stroke: #e53e3e; }
...
.pd-stock--in  { background: #edfaf3; color: var(--green); }
.pd-stock--out { background: #fdecea; color: var(--red); }
...
.pd-stars { color: #f4a623; font-size: 16px; letter-spacing: 2px; }
.pd-rating-count { font-size: 13px; color: var(--ink-mid); }
...
.pd-color-swatch:hover > span:first-of-type { transform: scale(1.15); }
.pd-color-swatch--active > span:first-of-type { border-color: var(--gold); }
.pd-color-name { font-size: 11px; color: var(--ink-mid); }
...
.pd-qty__btn:hover { background: var(--gold-light); }
.pd-qty__val {
  min-width: 52px;
  text-align: center;
  font-size: 16px;
...
.pd-btn--cart { background: var(--white); color: var(--gold-dark); }
.pd-btn--cart:hover { background: var(--gold); color: var(--white); }
.pd-btn--buy  { background: var(--gold); color: var(--white); }
.pd-btn--buy:hover { background: var(--gold-dark); transform: translateY(-2px); box-shadow: 0 6px 20px rgba(168,135,74,.3); }
...
.pd-trust__item svg { width: 22px; height: 22px; color: var(--gold); }
.pd-trust__item span { font-size: 11px; color: var(--ink-mid); font-weight: 500; }
...
.pt-tab:hover { color: var(--gold); }
.pt-tab--active { color: var(--gold-dark); border-bottom-color: var(--gold); }
...
.pt-table tr { border-bottom: 1px solid var(--border); }
.pt-table tr:last-child { border-bottom: none; }
.pt-table td { padding: 14px 16px; color: var(--ink-mid); }
...
.rv-big-stars { color: #f4a623; font-size: 24px; margin: 8px 0; }
.rv-total { font-size: 13px; color: var(--ink-light); margin-bottom: 20px; }
...
.rv-bar-row > span:first-child { width: 26px; flex-shrink: 0; }
.rv-bar-row > span:last-child  { width: 20px; text-align: right; flex-shrink: 0; }
.rv-bar {
  flex: 1;
  height: 8px;
  background: var(--border);
...
.rv-fill { height: 100%; border-radius: 4px; transition: width .6s ease; }

.rv-cards-col { display: flex; flex-direction: column; gap: 20px; }
.rv-card {
  background: var(--ivory);
  border: 1px solid var(--border);
  border-radius: var(--radius);
...
.rv-card__header > div strong { display: block; font-size: 14px; color: var(--ink); }
.rv-date { font-size: 12px; color: var(--ink-light); }
.rv-card__stars { margin-left: auto; color: #f4a623; font-size: 14px; }
.rv-card p { font-size: 14px; color: var(--ink-mid); line-height: 1.7; }

/* ============================================================
   PRODUCT GRIDS
============================================================ */
.grid-section { padding: 60px 0; background: var(--ivory); }
...
.gs-card:hover { transform: translateY(-5px); box-shadow: var(--shadow-md); }
...
.gs-card:hover .gs-card__img-wrap img { transform: scale(1.05); }
...
.gs-card:hover .gs-card__overlay { opacity: 1; }
.gs-quick-view {
  background: var(--white);
  border: none;
  padding: 10px 22px;
...
.gs-quick-view:hover { background: var(--gold); color: var(--white); }
...
.gs-card__wish svg { width: 16px; height: 16px; }
.gs-card__wish:hover { color: #e53e3e; }
...
.gs-card__price { display: flex; align-items: baseline; gap: 8px; }
.gs-new-price {
  font-size: 16px;
  font-weight: 700;
  color: var(--gold-dark);
...
@media (max-width: 768px) {
  ...
  .pd-main-img { height: auto; max-height: 480px; }
  ...
  .pd-trust__item { flex: 1 1 40%; }

  /* Tabs */
  .pt-section { padding: 40px 0; }
  .pt-tabs { overflow-x: auto; }
  .pt-tab { white-space: nowrap; padding: 12px 18px; font-size: 11px; }

  .pt-info-grid { grid-template-columns: 1fr; gap: 20px; }
  ...
  .gs-card__img-wrap img { height: 200px; }
  .gs-card__name { font-size: 13px; }
  .gs-new-price { font-size: 14px; }
  .gs-old-price { font-size: 12px; }
  ...
  /* Breadcrumb */
  .pd-breadcrumb { font-size: 11px; padding-top: 16px; margin-bottom: 16px; }
...
@media (max-width: 420px) {
  ...
  .pd-sizes { gap: 6px; }
  .pd-size { min-width: 38px; height: 36px; font-size: 12px; }
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.querySelectorAll('.pd-size').forEach(btn => {
    btn.addEventListener('click', function () {
      document.querySelectorAll('.pd-size').forEach(b => b.classList.remove('pd-size--active'));
  ...
  document.querySelectorAll('.pd-color-swatch').forEach(sw => {
    sw.addEventListener('click', function () {
      document.querySelectorAll('.pd-color-swatch').forEach(s => s.classList.remove('pd-color-swatch--active'));
  ...
  document.querySelectorAll('.pt-tab').forEach(tab => {
    tab.addEventListener('click', function () {
      ...
      document.getElementById('tab-' + this.dataset.tab)?.classList.add('pt-panel--active');
    });
  });
